  ...
  switch (type) {
    ...
    default:
      oauth.getProtectedResource(
        ...
        {
          if( err ){
            console.log('FantasyRequest.prototype._request error:');
            console.log( err );
            return;
          }

          //  assumes we've asked for XML, so convert to JSON
          //var parser = new xml2js.Parser();
          //json = parser.parseString(data);

          //  this way expects ?format=json from Y! to work flawlessly
          var json = typeof data === "string" ? JSON.parse(data) : data;
